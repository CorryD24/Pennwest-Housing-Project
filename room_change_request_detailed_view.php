<?php
$title = "Room Request Change Detailed View";
//This page will have the detailed view of a room change request
require_once '../view/header.php'; //This requires the header
// First, check if user is logged ini
if (!isset($_SESSION['FirstName'])) {
    $errorMessage = 'Please log in to view the request.';
                ...
                <div class="col-lg-12">
                    <div class="table-responsive">
                        <table class="table" style="margin-top: 10px;">
                            ...
                            <thead class="font15">
                                <tr>
                                    <th colspan="5" scope="col">Reason</th>
                                </tr>
                            </thead>
                            <tbody class="font13">
                                <tr>
                                    <td colspan="5" scope="row"><?php echo htmlspecialchars($row['reason']); ?></td>
                                </tr>
                            </tbody>
                            <thead class="font15">
                                <tr>
                ...
                <div class="row">
                    <div class="col-lg-12 centerColumnGray">
                        ...
                        <?php if ($_SESSION['Email'] == $row['email']) { //User that is logged in email must match the email in the row being displayed
                            ...
                            <input type="button" class="btn btn-outline-success btnMarginB" name="EditChangeButton"
                                id="EditChangeButton" value="Edit Request"
                                onclick="document.location='../controller/controller.php?action=EditRoomChangeRequest&RoomChangeRequestID=<?php echo $row['id'] ?>';">
                        <?php }
                            ...
                            <input type="button" style="float: right;" class="btn btn-outline-danger btnMarginB"
                                name="DeleteChangeButton" id="DeleteChangeButton" value="Delete Request"
                                onclick="conformation_change()">
                        <?php } else if (in_array('AG-ROLE-STAFF', $_SESSION['Role'])) { ?>
                                <!-- Staff buttons, delete, bump to bottom, mark as complete, send email-->
                                ...
                                <input type="button" class="btn btn-outline-primary btnMarginB" name="SendEmailButton"
                                    ...
                                    onclick="document.location='../controller/controller.php?action=SendEmail&Form=RoomChange&ReqID=<?php echo $row['id'] ?>&Email=<?php echo $row['email'] ?>&FirstName=<?php echo $row['first_name'] ?>&PreferredName=<?php echo $row['preferred_name'] ?>&LastName=<?php echo $row['last_name'] ?>&DateTime=<?php echo $row['email_datetime'] ?>'">
                                ...
                                <input type="button" class="btn btn-outline-primary btnMarginB" name="BumpToBottomButton"
                                    ...
                                    onclick="bump_to_bottom_confirmation_change()"><!--Call the confirmation fctn-->
                                <!--We must send the row to the controller for this feature as we are deketeing and reuploading the request so it gets
                                the newwest id number and therefore displayed last when sorting normally.-->

                                <!--Change Completition Status Button-->
                                <input type="button" class="btn btn-outline-success btnMarginB" name="ChangeCompletition"
                                    id="ChangeCompletition" <?php
                                    ...
                                    onclick="document.location = '../controller/controller.php?action=ChangeRoomChangeRequestCompleted&RoomChangeRequestID=<?php echo $row['id']; ?>'">

                                <!-- Delete request button-->
                                <input type="button" style="float: right;" class="btn btn-outline-danger btnMarginB"
                                    name="DeleteChangeButton2" id="DeleteChangeButton2" value="Delete Request"
                                    onclick="admin_conformation_change()">
                            ...
                            <?php if ($row['deleted'] == 'Y') { ?>
                                    <input type="button" style="float: right; margin-right: 3px;"
                                        ...
                                        value="Undo Delete" onclick="undo_confirmation_change()">
...
<script>
    /**
     * This function confirms that the student wants to delte the request. If yes it navigates to the controller to do so, otheriwse it ends.
     */
    function conformation_change() {
        if (confirm("Are you sure you want to delete the request?") === true) {
            document.location.href = '../controller/controller.php?action=DeleteRoomChangeRequest&RoomChangeRequestID=<?php echo $row['id'] ?>&DeleteType=Permanate';
        }
        else { //Dont delete
            exit;
        }
     ...
     */
    function admin_conformation_change() {
        ...
        if (deletedRow == 'N') {//If the row hasn't been 'deleted' as in pretend deleted we want to pretend to delete it
            if (confirm("Are you sure you want to delete the request?") === true) {
                document.location.href = '../controller/controller.php?action=DeleteRoomChangeRequest&RoomChangeRequestID=<?php echo $row['id'] ?>&DeleteType=Pretend';
            }
     ...
     */
    function bump_to_bottom_confirmation_change() {
        if (confirm("Are you sure you want to bump this request to the bottom of the queue?") === true) {
            document.location.href = '../controller/controller.php?action=BumpRoomChangeRequestToBottom&Row=<?php echo $row['id']; ?>';
        }
        else {
     ...
     */
    function undo_confirmation_change() {
        if (confirm("Are you sure that you'd like to recover this request?") === true) {
            document.location.href = '../controller/controller.php?action=UndoDeleteRoomChangeRequest&RoomChangeRequestID=<?php echo $row['id'] ?>';
